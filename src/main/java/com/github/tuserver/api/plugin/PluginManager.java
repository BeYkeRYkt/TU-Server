...
import com.github.tuserver.api.event.EventException;
import com.github.tuserver.api.event.EventExecutor;
import com.github.tuserver.api.event.EventHandler;
import com.github.tuserver.api.event.EventPriority;
import com.github.tuserver.api.event.HandlerList;
import com.github.tuserver.api.event.Listener;
import com.github.tuserver.api.event.RegListener;
...
public class PluginManager {
	...
	public void loadPlugins() {
		File pluginsFolder = TUServer.getPluginsFolder();

		plugins = new ArrayList<Plugin>();
		prefiles = new ArrayList<PluginDescriptionFile>();

		TUServer.getServer().getLogger().info("Loading plugins...");
		for (File file : pluginsFolder.listFiles()) {
			if (file.getName().toLowerCase().endsWith(".jar") && (!file.isDirectory())) {
				...
				try {
					...
					TUServer.getServer().getLogger().error("Failed to load plugin " + file.getName());
					e.printStackTrace();
				}
			} else if (!file.getName().toLowerCase().endsWith(".jar") && (!file.isDirectory())) {
				TUServer.getServer().getLogger().info("Skipping " + file.getName() + " not a plugin.");
			}
		}
	...
	public void enablePlugins() {
		TUServer.getServer().getLogger().info("Starting plugins");
	...
	public void disablePlugins() {
		TUServer.getServer().getLogger().info("Disabling plugins");
	...
	public void loadPlugin(PluginDescriptionFile pFile) {
		try {
			...
			if (instance instanceof Plugin) {
				...
				plugin.onLoad();
				plugins.add(plugin);
				TUServer.getServer().getLogger().info("Loaded " + pFile.getName());
			} else {
				TUServer.getServer().getLogger().info("Skipping " + pFile.getName() + " not a plugin.");
			}

		} catch (Exception e) {
			TUServer.getServer().getLogger().error("Failed to load plugin " + pFile.getName() + ".");
			e.printStackTrace();
		}
	}

	public void enablePlugin(Plugin plugin) {
		TUServer.getServer().getLogger().info("Starting " + plugin.getFileDescription().getName());
		plugin.enable = true;
		plugin.onEnable();
	}

	public void disablePlugin(Plugin plugin) {
		TUServer.getServer().getLogger().info("Disabling " + plugin.getFileDescription().getName());
		plugin.enable = false;
		...
		if (listeners != null) {
			for (RegListener listener : listeners) {
				...
				try {
					...
					TUServer.getLogger().error("Could not pass event " + event.getEventName() + " to " + listener.getOwner().getClass().getName(), ex);
				}
			}
		}
		return event;
	...
	public void registerEvents(Listener listener, Plugin owner) {
		for (Map.Entry<Class<? extends Event>, Set<RegListener>> entry : createRegisteredListeners(listener, owner).entrySet()) {
			...
			if (!entry.getKey().equals(delegatedClass)) {
				TUServer.getLogger().error("Plugin attempted to register delegated event class " + entry.getKey() + ". It should be using " + delegatedClass + "!");
				continue;
			}
	...
	public Map<Class<? extends Event>, Set<RegListener>> createRegisteredListeners(final Listener listener, Plugin plugin) {
		...
		try {
			...
			TUServer.getLogger().error("Plugin " + plugin.getClass().getSimpleName() + " is attempting to register event " + e.getMessage() + ", which does not exist. Ignoring events registered in " + listener.getClass());
			return ret;
		}
		for (final Method method : methods) {
			...
			if (!Event.class.isAssignableFrom(checkClass) || method.getParameterTypes().length != 1) {
				TUServer.getLogger().error("Wrong method arguments used for event type registered");
				continue;
			} else {
